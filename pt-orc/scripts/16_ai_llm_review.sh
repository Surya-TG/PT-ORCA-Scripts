...
# =============================================================================
# 16_ai_llm_review.sh — TechGuard. [VAPT-Advanced v3.0 — 2026-06-16]
...
emit_finding() {
    ...
    local payload
    payload=$(printf '{"id":"%s","title":"%s","severity":"%s","phase":"16_ai_llm","evidence_ids":["%s"],"description":"%s","recommendation":"%s","retest_status":"n/a","residual_risk":""}' \
        "$fid" \
        "$(echo "$title" | sed 's/"/\\"/g')" \
        "$sev" \
        "${ev_tag:-$ev_id}" \
...
main() {
    log "PT-Orc 16_ai_llm_review.sh v3.0 — OWASP LLM Top 10 (2025) — 21 tests + dynamic harvest"
    ...
    if command -v trail_phase_start &>/dev/null; then
        trail_phase_start "16_ai_llm" "AI/LLM Review v2.0" "${#targets[@]} targets"
    fi
    ...
    local summary_md="${SCRIPT_DIR}/working/$(ev_fname "09-llm-summary" "md")"
    {
        ...
        echo ""
        echo "---"
        echo "*Generated by PT-Orc 16_ai_llm_review.sh v3.0 — TechGuard Labs*"
    ...
    if command -v trail_phase_end &>/dev/null; then
        trail_phase_end "16_ai_llm" "${_FIND_CTR} findings" "$summary_md"
    fi
