...
# =============================================================================
# 14_app_api_review.sh — TechGuard. [VAPT-Advanced v2.0 — 2026-06-09]
    ...
    for entry in "${manifest_map[@]}"; do
        ...
        for pkg in "${pkgs[@]+"${pkgs[@]}"}"; do
            ...
            emit_package_finding "info" \
                ...
                "Package '${pkg}' (ecosystem: ${eco}) identified from exposed /${mfile} at ${ip}:${port}. Step 23 (Vuln Corpus) will query OSV.dev for known vulnerabilities in this package." \
...
main() {
    log "PT-Orc 14_app_api_review.sh v2.0 — OWASP API Top 10 + Advanced Attacks"
    ...
    if command -v trail_phase_start &>/dev/null; then
        trail_phase_start "14_app_api" "App/API Review v2.0" "${#targets[@]} targets"
    fi
    ...
    local summary_md="${SCRIPT_DIR}/working/$(ev_fname "14-appapi-summary" "md")"
    {
        ...
        echo ""
        echo "---"
        echo "*Generated by PT-Orc 14_app_api_review.sh v2.1 — TechGuard Labs*"
